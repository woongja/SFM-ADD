...
from model.loss_metrics import augmentation_aware_supcon_loss, supcon_loss, sim_metric_seq
...
def eval_model(args, config, device):
    ...
    if isinstance(eval_result, tuple):
        # data_utils_balance/balance_scl returns (file_list, noise_dict)
    ...
    # Check if using SCL model
    is_scl_model = (config["model"]["name"] == "conformertcm_scl")
    ...
    with torch.no_grad(), open(args.eval_output, 'w') as fh:
        for batch_x, utt_id in tqdm(eval_loader, desc="Evaluation", leave=False):
            ...
            # Model forward
            if is_scl_model:
                # SCL model returns (logits, attn_score, embedding, feats)
                logits, _, _, _ = model(batch_x)
            else:
                # Standard model returns (logits, attn_score)
...
# Training Functions
############################################
def train_epoch(train_loader, model, optimizer, device, epoch, use_balance_training=True, use_curriculum=False, curriculum_stage=1, config=None):
    ...
    # Check if SCL is enabled
    use_scl = False
    scl_config = None
    is_scl_model = False
    if config is not None:
        # Check if using SCL model (returns 4 values)
        is_scl_model = (config.get('model', {}).get('name', '') == 'conformertcm_scl')
        # Check if contrastive learning is enabled
        if 'contrastive_learning' in config:
            scl_config = config['contrastive_learning']
            use_scl = scl_config.get('enabled', False)
    ...
    if use_balance_training or use_curriculum:
        ...
        if use_curriculum:
            ...
            # Use SCL version that returns augmentation types
            data_module = importlib.import_module("datautils.data_utils_balance_scl")
        else:
        ...
        for batch_idx, batch_info in enumerate(pbar):
            ...
            if log_aug:
                if use_curriculum:
                    ...
                    batch_x, batch_y, batch_aug_types, batch_aug_stats = online_augmentation_collate_fn(
                ...
                if use_curriculum:
                    ...
                    batch_x, batch_y, batch_aug_types = online_augmentation_collate_fn(
                        batch_info, base_dir, args)
                else:
            ...
            # Model forward pass
            if use_scl:
                # Model returns (logits, attn_score, embedding, feats) for SCL
                logits, _, embeddings, feats = model(batch_x)
            else:
                # Model returns (logits, attn_score)
                logits, _ = model(batch_x)

            # Calculate loss
            if use_scl and batch_aug_types is not None:
                real_bsz = logits.shape[0]

                # Get SCL parameters
                temperature = scl_config.get('temperature', 0.07)
                contra_mode = scl_config.get('contra_mode', 'all')
                weighted_loss = scl_config.get('weighted_loss', False)
                loss_type = scl_config.get('loss_type', 1)

                # 1. Cross-Entropy Loss (L_CE)
                L_CE = criterion(logits, batch_y)

                # 2. Contrastive Loss on features (L_CF1)
                feats_expanded = feats.unsqueeze(1)  # [bsz, 1, frame_num, emb_dim]
                if weighted_loss:
                    same_aug_weight = scl_config.get('same_aug_weight', 0.3)
                    diff_aug_weight = scl_config.get('diff_aug_weight', 0.7)
                    L_CF1 = augmentation_aware_supcon_loss(
                        feats_expanded,
                        labels=batch_y,
                        aug_types=batch_aug_types,
                        t=temperature,
                        contra_mode=contra_mode,
                        length_norm=True,
                        same_aug_weight=same_aug_weight,
                        diff_aug_weight=diff_aug_weight
                    )
                else:
                    L_CF1 = supcon_loss(
                        feats_expanded,
                ...
                # 3. Contrastive Loss on embedding (L_CF2)
                emb_expanded = embeddings.unsqueeze(1).unsqueeze(-1)  # [bsz, 1, emb_dim, 1]
                if weighted_loss:
                    L_CF2 = augmentation_aware_supcon_loss(
                        emb_expanded,
                        labels=batch_y,
                        aug_types=batch_aug_types,
                        t=temperature,
                ...
                if loss_type == 1:
                    # L_CE + L_CF1 + L_CF2 (all three)
                    loss = L_CE + L_CF1 + L_CF2
                elif loss_type == 2:
                    # L_CE + L_CF1
                    loss = L_CE + L_CF1
                    L_CF2 = torch.tensor(0.0)
                elif loss_type == 3:
                    # L_CE + L_CF2
                    loss = L_CE + L_CF2
                    L_CF1 = torch.tensor(0.0)
                elif loss_type == 4:
                    # L_CE only
                    loss = L_CE
                    L_CF1 = torch.tensor(0.0)
                    ...
                    # L_CF1 + L_CF2 (no CE)
                    loss = L_CF1 + L_CF2
                    L_CE = torch.tensor(0.0)
                else:
                    # Default: all three
                    loss = L_CE + L_CF1 + L_CF2

                running_ce_loss += L_CE.item()
                running_contrast_loss += (L_CF1.item() + L_CF2.item())
            else:
                # Standard CE loss
            ...
            if use_scl:
                postfix_dict['ce'] = f'{running_ce_loss / num_batches:.4f}'
                postfix_dict['contra'] = f'{running_contrast_loss / num_batches:.4f}'
...
def eval_epoch(dev_loader, model, device, epoch, use_balance_training=True, is_scl_model=False):
    model.eval()
    val_loss = 0.0
    correct, total = 0, 0
    num_batches = 0
...
def main(args):
    ...
    # Check if using SCL model (returns 4 values instead of 2)
    is_scl_model = (config["model"]["name"] == "conformertcm_scl")
    ...
    if use_curriculum:
        ...
        for stage_num, stage_epochs in stages:
            ...
            for stage_epoch in range(stage_epochs):
                ...
                tr_loss, tr_acc = train_epoch(train_loader, model, optimizer, device, epoch,
                                             ...
                                             curriculum_stage=stage_num, config=config)

                # Validate
                val_loss, val_acc = eval_epoch(dev_loader, model, device, epoch, use_balance_training=True, is_scl_model=is_scl_model)
        ...
        for epoch in range(args.start_epoch, args.num_epochs):
            ...
            # Train
            tr_loss, tr_acc = train_epoch(train_loader, model, optimizer, device, epoch, use_balance_training, config=config)

            # Validate
            val_loss, val_acc = eval_epoch(dev_loader, model, device, epoch, use_balance_training, is_scl_model=is_scl_model)
