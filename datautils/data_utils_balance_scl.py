"""
Data utilities for Balanced Training with Supervised Contrastive Learning (SCL)

This module extends data_utils_balance.py to support augmentation-aware contrastive learning.
Key features:
- Returns augmentation type information in collate_fn
- Supports weighted contrastive loss based on augmentation similarity
"""
...
def online_augmentation_collate_fn(batch_info, base_dir, args, log_augmentation=False):
    """
    Custom collate function for online augmentation with Supervised Contrastive Learning (SCL)
    ...
    Returns:
        ...
        batch_aug_types: List of augmentation types (for augmentation-aware contrastive loss)
    ...
    # batch_noise_list contains augmentation types for each sample in the batch
    # This will be used for augmentation-aware contrastive learning

    if log_augmentation:
        # Return with augmentation statistics
        return batch_x, batch_y, batch_noise_list, dict(aug_stats)
    else:
        # Return augmentation types for augmentation-aware contrastive loss
